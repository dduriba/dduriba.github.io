...
<br>(그룹화하면 Sprite Renderer 를 가진 하위 게임오브젝트들끼리의 Sprite 정렬은 Sprite Renderer 의 Order in Layer 값에 따라 결정되고 다른 게임오브젝트들과의 Sprite 정렬은 Sotring Group 의 Order in Layer 값에 따라 결정돼 하나의 Sprite 처럼 동작)
</div>
