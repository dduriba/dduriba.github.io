...
arrayList.add("data"); // 데이터 삽입
arrayList.add(0, "data"); // index에 데이터 삽입, 기존 값들은 뒤로 밀림
arrayList.addAll(arrayList); // 기존 값들을 유지한 채 인자로 넣은 컬렉션의 데이터 복제 삽입
```
...
```java
List<String> arrayList = new ArrayList<>(10); // 초기 용량 설정

arrayList.size(); // 리스트의 크기 반환, 현재 크기는 0이기 때문에 0 반환
```