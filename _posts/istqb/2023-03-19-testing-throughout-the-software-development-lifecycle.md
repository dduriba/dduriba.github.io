...
<u>컴포넌트 테스팅과 마찬가지로, 자동 통합 리그레션 테스트를 수행하여 수정으로 인해 기존 인터페이스, 컴포넌트, 시스템 등이 손상되지 않았다는 확신을 얻는 경우가 있다.</u><br>
...
다음은 화이트박스 테스트의 예이다:
- 컴포넌트 테스팅에 적용, 금융 계산을 수행하는 모든 컴포넌트에 대한 완벽한 구문 및 결정 커버리지(4.3 장 참조)를 달성하기 위한 테스트 설계
