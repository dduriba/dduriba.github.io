...
2. 테스트대상의식별- 개발자가 이미 이 문제에 대해 알고 있고 당신은 시스템 테스팅을 수행하고 있다. 또한 시스템 버전도 이미 제공했으므로 개발자들도 테스트한 대상을 알고 있을 거라고 볼 수 있다. 따라서 이 정보는 다른 정보에 비해 덜 중요하다.
...
- n. 스크린에 보이는 잘못된 합계
- o. 인수 테스팅 도중 인수 조건 변경
- p. 사용자가 앱에서 소프트 키보드 사용이 너무 어렵다고 느낌
- q. 검색어 입력 중 시스템이 사용자 입력에 너무 느리게 응답함
- r. 일일 스탠딩 회의에서 테스터의 결과 보고가 금지되어 있음
...
- n. 스크린에 보이는 잘못된 합계 오류: 제품 리스크
- o. 인수 테스팅 도중 인수 조건 변경: 프로젝트 리스크
- p. 사용자가 앱과 함께 소프트 키보드를 사용하기가 너무 어려움: 제품 리스크
- q. 검색어 입력 중 시스템이 사용자 입력에 너무 느리게 응답함: 제품 리스크
- r. 일일 스탠딩 회의에서 테스터가 테스트 결과를 보고하지 못함: 프로젝트 리스크
