...
```html
<!-- 블록(Block): 블록 요소는 항상 새로운 줄에서 시작하며, 가능한 최대 가로 너비를 차지합니다. -->
<!-- 블록 요소는 부모 요소의 전체 너비를 차지하며, 수직으로 쌓이기 때문에 다른 요소들이 옆에 위치할 수 없습니다. -->
<!-- 대표적인 블록 요소로는 div, p, h1부터 h6, form 등이 있습니다. -->
<div>
  This is a block element. It takes up the full width available.
</div>

<!-- 인라인(Inline): 인라인 요소는 새로운 줄에서 시작하지 않고, 요소가 끝날 때까지 수평으로 차지하는 너비만큼만 영역을 차지합니다. -->
